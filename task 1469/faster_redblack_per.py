...
n = int(fast_input())
for i in range(n):
    x1,y1,x2,y2 = map(int,fast_input().split())
...
def solve(segments):
    for i in range(len(events)):
        ...
        if events[i].type == -1:
            # checking existense of key in tree
            perpendicular = None
            if t.search(key)._key is not None:
                perpendicular = t.search(key).value
            # end checking existense of key in tree
            ...
            if perpendicular and intersect(perpendicular,element):
                return element,perpendicular
            #end serach
